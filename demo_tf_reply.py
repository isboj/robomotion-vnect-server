...
def message_received(client, server, message):
    ...
    print(2, type(message))
    message = np.fromstring(message,dtype=np.uint8)#バイトデータ→ndarray変換
    print(2.5, message.shape)
    message = np.reshape(message,(480,640,3))#形状復元(これがないと一次元行列になってしまう。)　reshapeの第二引数の(480,640,3)は引数は送られてくる画像の形状
    print(3, type(message))
    print(4, message.shape)
    
    #vnectで取得
    message = get_vnect_joints(message)
    message = message.astype(np.uint8)
    # 元の形式まで戻った
    message = message.tostring()  # numpyからバイトデータに変換
    message = base64.b64encode(message).decode('utf-8')  # デコード
